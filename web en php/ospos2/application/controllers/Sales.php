...
class Sales extends Secure_Controller
{
	...
	public function complete()
	{
		...
		$data['company_info'] = implode("\n", array(
			...
			"<b>Direccion: </B>".$this->config->item('address'),
			 
			"<b>RUC: </B>".$this->config->item('ruc'),
				 
				"<b>Telefono: </B>".$this->config->item('phone')
		));
		
		
		$data['company_rucs'] = implode("", array(
		 
			 
				$this->config->item('ruc')
	...
	private function _load_customer_data($customer_id, &$data, $stats = FALSE)
	{
		...
		if($customer_id != -1)
		{
			...
			$data['customer_ruc'] = $customer_info->ruc_cliente;
	...
	private function _load_sale_data($sale_id)
	{
		...
		$data['company_info'] = implode("", array(
		"<b>Direccion: </B>",
