	...
	public function get_search_suggestions($search, $unique = TRUE, $limit = 25)
	{
		...
		$this->db->group_start();
			...
			if($unique)
			{
				...
				$this->db->or_like('ruc_cliente', $search);
		...
		{
			$suggestions[] = array('value' => $row->person_id, 'label' => $row->first_name . ' ' . $row->last_name . (!empty($row->company_name) ? ' [' . $row->company_name . ']' : ''). (!empty($row->ruc_cliente) ? ' [' . $row->ruc_cliente . ']' : ''));
		}
